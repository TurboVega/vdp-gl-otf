/*
  Painter64 - Paints (draws) using 64 colors
...
std::function<void(int X, int Y, uint8_t colorIndex)> Painter64::setPixelLambda(PaintMode mode) {
  switch (mode) {
    case PaintMode::Set:
...
std::function<void(uint8_t * row, int x, uint8_t colorIndex)> Painter64::setRowPixelLambda(PaintMode mode) {
  switch (mode) {
    case PaintMode::Set:
...
std::function<void(int Y, int X1, int X2, uint8_t colorIndex)> Painter64::fillRowLambda(PaintMode mode) {
  switch (mode) {
    case PaintMode::Set:
...
void Painter64::setPixelAt(PixelDesc const & pixelDesc, Rect & updateRect) {
...
void Painter64::absDrawLine(int X1, int Y1, int X2, int Y2, RGB888 color) {
...
void Painter64::fillRow(int y, int x1, int x2, RGB888 color) {
...
// parameters not checked
void Painter64::rawFillRow(int y, int x1, int x2, uint8_t pattern) {
...
// parameters not checked
void Painter64::rawORRow(int y, int x1, int x2, uint8_t pattern) {
  auto row = m_viewPort[y];
...
// parameters not checked
void Painter64::rawANDRow(int y, int x1, int x2, uint8_t pattern) {
  auto row = m_viewPort[y];
  // naive implementation - just do whole row iteratively
  for (int x = x1; x <= x2; ++x) {
    VGA_PIXELINROW(row, x) &= pattern;
...
// parameters not checked
void Painter64::rawXORRow(int y, int x1, int x2, uint8_t pattern) {
  auto row = m_viewPort[y];
  // naive implementation - just do whole row iteratively
  for (int x = x1; x <= x2; ++x) {
    VGA_PIXELINROW(row, x) ^= pattern;
...
// parameters not checked
void Painter64::rawInvertRow(int y, int x1, int x2) {
  auto row = m_viewPort[y];
  for (int x = x1; x <= x2; ++x) {
...
void Painter64::rawCopyRow(int x1, int x2, int srcY, int dstY) {
    // This function does not seem to be used
}

void Painter64::swapRows(int yA, int yB, int x1, int x2) {
...
void Painter64::VScroll(int scroll, Rect & updateRect) {
...
void Painter64::HScroll(int scroll, Rect & updateRect) {
...
void Painter64::drawGlyph(Glyph const & glyph, GlyphOptions glyphOptions, RGB888 penColor, RGB888 brushColor, Rect & updateRect) {
  auto mode = paintState().paintOptions.mode;
...
void Painter64::invertRect(Rect const & rect, Rect & updateRect) {
  genericInvertRect(rect, updateRect,
                    [&] (int Y, int X1, int X2) { rawInvertRow(Y, X1, X2); }
                   );
}

void Painter64::swapFGBG(Rect const & rect, Rect & updateRect) {
...
void Painter64::copyRect(Rect const & source, Rect & updateRect) {
...
void Painter64::rawDrawBitmap_Mask(int destX, int destY, Bitmap const * bitmap, void * saveBackground, int X1, int Y1, int XCount, int YCount) {
  auto paintMode = paintState().paintOptions.mode;
...
void Painter64::rawDrawBitmap_RGBA2222(int destX, int destY, Bitmap const * bitmap, void * saveBackground, int X1, int Y1, int XCount, int YCount) {
  auto paintMode = paintState().paintOptions.mode;
  auto setRowPixel = setRowPixelLambda(paintMode);
...
void Painter64::rawCopyToBitmap(int srcX, int srcY, int width, void * saveBuffer, int X1, int Y1, int XCount, int YCount) {
...
void Painter64::rawDrawBitmapWithMatrix_Mask(int destX, int destY, Rect & drawingRect, Bitmap const * bitmap, const float * invMatrix) {
  auto paintMode = paintState().paintOptions.mode;
  auto setRowPixel = setRowPixelLambda(paintMode);
  auto getPixel = getPixelLambda(paintMode);
  auto pattern = getPixel(paintState().paintOptions.swapFGBG ? paintState().penColor : bitmap->foregroundColor);
...
void Painter64::rawDrawBitmapWithMatrix_RGBA2222(int destX, int destY, Rect & drawingRect, Bitmap const * bitmap, const float * invMatrix) {
  auto paintMode = paintState().paintOptions.mode;
  auto setRowPixel = setRowPixelLambda(paintMode);
...
void Painter64::rawDrawBitmapWithMatrix_RGBA8888(int destX, int destY, Rect & drawingRect, Bitmap const * bitmap, const float * invMatrix) {
  auto paintMode = paintState().paintOptions.mode;
  auto setRowPixel = setRowPixelLambda(paintMode);
